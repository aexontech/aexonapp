...
- Kop Surat (Hospital Letterhead): Personal accounts: up to 3 kop surat in Settings→Kop Surat tab. Each has: name, logo file upload (.jpg/.png, max 5MB) with crop/zoom modal (react-easy-crop), address, phone, fax, email, website. Logo stored as base64 data URL in logoUrl field. Name/logo have 14-day cooldown after change. Dokter Institusi: read-only view of institution kop in same tab. Admin Institusi: manages single institution kop via standalone AdminKopSurat page. ReportGenerator: Personal gets dropdown selector, enterprise users get locked auto-select.
