...
- Auth & Subscription: All auth and subscription validation routed through AEXON Connect API (`src/lib/aexonConnect.ts`). Uses `VITE_AEXON_CONNECT_API_URL` env var. JWT token stored in sessionStorage (or localStorage if remember-me). Endpoints: POST /auth/login, POST /auth/register, POST /auth/reset-password, POST /auth/change-password, PUT /auth/profile, GET /subscription/status, GET /plans, POST /subscription/checkout (sends plan_id + device_id only), POST /subscription/promo/validate, POST /device/register, POST /device/verify, GET /subscription/billing-history, POST /auth/logout. No direct Supabase access for billing or subscription. `src/lib/supabase.ts` still exists but is not imported by any component.
- Device ID: Auto-generated UUID stored in localStorage (`aexon_device_id`), sent with checkout and device verification requests.
...
- Encryption: XOR cipher in `src/lib/storage.ts` (saveUserData/loadUserData). Gallery "Export Case" creates encrypted ZIP (session.enc + manifest.enc + media/ blobs) using same XOR key; restored via Settings Backup & Restore (detects manifest.enc → decrypts → rebuilds captures with blob URLs). Case export file: `Aexon_Case_[RM]_[date].zip`.
- Subscription: subscription-only model (no tokens), gating via hasActiveAccess. Full in-app checkout flow connected across entire system. Entry points: Dashboard CTA banner, sidebar "Perpanjang Sekarang" button, Settings Langganan tab, AdminDashboard subscription banner, ManageSubscription "Perpanjang" button. All route to PlanSelection → Checkout. PlanSelection fetches plans from AEXON Connect GET /plans. Checkout sends POST /subscription/checkout with plan_id + device_id (backend determines price and creates invoice). Post-checkout: `onSuccess` callback triggers `handleCheckoutSuccess` in App.tsx which calls `refreshSubscriptionStatus()` via AEXON Connect GET /subscription/status. Promo validation via POST /subscription/promo/validate. Components: PlanSelection.tsx, Checkout.tsx.
