...
- Light theme everywhere: Gallery.tsx = light #F8FAFC bg with white cards, tab pills, multi-select checkboxes, bottom export bar. EndoscopyApp.tsx = light header, dark camera viewport (#1a1a2e with borderRadius 20), white sidebar with collapsible session info, bottom control bar with capture/record buttons, keyboard shortcuts hint. Drag-select uses `[data-gallery-item]` attribute.
...
- Subscription: subscription-only model (no tokens), gating via hasActiveAccess. Full in-app checkout flow connected across entire system (Aexon Web = personal doctor UI, Aexon Hub = admin/institutional UI). Entry points: Dashboard CTA banner, sidebar "Perpanjang Sekarang" button, Settings Langganan tab, AdminDashboard subscription banner, ManageSubscription "Perpanjang" button. All route to PlanSelection → Checkout → Xendit payment. Post-checkout: `onSuccess` callback triggers `handleCheckoutSuccess` in App.tsx which optimistically sets `selectedPlan='subscription'` + triggers `refreshSubscriptionStatus()` to re-verify from Supabase. Checkout layout: centered (maxWidth 960, white order summary card). Xendit invoice creation via api-server `POST /api/xendit/create-invoice` (raw HTTP, no SDK). Components: PlanSelection.tsx, Checkout.tsx.
- Settings (Settings.tsx): 5-tab layout (Profil / Keamanan / Kop Surat / Langganan / Backup). Profil: avatar initials, name, email(read-only), specialization, STR, SIP, phone — saves to Supabase doctor_accounts via auth.getUser() UUID. Keamanan: change password via supabase.auth.updateUser({ password }), data cleanup. Kop Surat: Personal=editable up to 3 collapsible cards with anti-abuse protections, Dokter Institusi=read-only card, Admin=no tab. Langganan: Personal shows plan+billing+CTA with Supabase-fetched pricing from `product_plans` table (joined with `products(name)`), billing cycle toggle (Bulanan/Tahunan), dynamic plan cards, continues to payment-info modal; Dokter Institusi hides tab entirely; Admin shows enterprise plan+seat count. Backup: existing ZIP backup/restore with conflict resolution.
