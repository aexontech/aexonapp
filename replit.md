...
- Subscription: subscription-only model (no tokens), gating via hasActiveAccess. Full in-app checkout flow: Dashboard "Berlangganan Sekarang" → PlanSelection (fetches plans from Supabase with beta_mode filtering) → Checkout (Shopify-style split layout, promo code validation, Xendit payment gateway integration with fallback to manual bank transfer). Xendit invoice creation via api-server `POST /api/xendit/create-invoice` (raw HTTP, no SDK). Components: PlanSelection.tsx, Checkout.tsx.
